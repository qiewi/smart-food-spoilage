"""Same models as the internal test (train 70% per split, gas+env, new labels), but
evaluated on VALIDATION (full 19/81 and rebalanced 50/50), per split x model.

Training is identical to internal_test_tables.py (deterministic, same seed) so these are
literally "the same models" — only the test set changes from the internal 30% to validation.
Metrics macro (recall = overall).
"""
...
# ---- same 70/30 TRAIN indices as internal_test_tables.py ----
...
for tag, d in [("VALIDATION ASLI (19/81)", full), ("VALIDATION 50/50", bal)]:
    print("\n" + "#" * 92 + f"\n# GAS+ENV (label baru, model dari train-70%) — {tag}\n" + "#" * 92)
    for sname in TRAIN:
        tbl = pd.DataFrame({m: d[(sname, m)] for m in MODEL_NAMES}).T[COLS]
        print(f"\n--- {sname} ---")
        print(tbl.round(3).to_string())

pd.DataFrame(full).T.to_csv(config.RESULTS_DIR / "gasenv_validation_full_newlabel.csv")
pd.DataFrame(bal).T.to_csv(config.RESULTS_DIR / "gasenv_validation_5050_newlabel.csv")
print("\nTersimpan: results/gasenv_validation_full_newlabel.csv & gasenv_validation_5050_newlabel.csv")
