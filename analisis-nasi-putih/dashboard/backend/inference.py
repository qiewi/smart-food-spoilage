"""Muat pipeline (split x model) GAS-ONLY Nasi Putih dan jalankan prediksi.

Tiap .joblib = Pipeline sklearn lengkap (StandardScaler + classifier) pada 3 sinyal gas
[mq2, mq135, mq4]. `food_type` TIDAK dipakai (single-food) tetapi tetap diterima sebagai
argumen agar kompatibel dengan frontend/dashboard lama.
"""
...
def predict(split, model, food_type, mq2, mq135, mq4):
    ...
    row = pd.DataFrame([{"mq2": mq2, "mq135": mq135, "mq4": mq4}])   # gas-only; food_type diabaikan
