"""GAS+ENV internal test: train on the split's 70%, evaluate on its own 30% test.

Same as gas_only_internal_test.py but with the full 5 features (mq2/mq135/mq4 + humidity +
tempC + food_type). Three 70/30 splits of training/: Stratified K-Fold, Grouped (whole
runs, test trimmed trailing keeping both classes), Stratified per Trial. Each model tuned
on the 70% train, evaluated on the 30% internal test. Metrics macro (recall = overall).
"""
...
print("\n" + "#" * 92 + "\n# GAS+ENV — UJI INTERNAL (train 70% -> test 30% dari data training)\n" + "#" * 92)
for sname in SPLITS:
    tbl = pd.DataFrame({m: res[(sname, m)] for m in MODEL_NAMES}).T[COLS]
    print(f"\n--- {sname} ---")
    print(tbl.round(3).to_string())

pd.DataFrame(res).T.to_csv(config.RESULTS_DIR / "internal_test_tables.csv")
print("\nTersimpan: results/internal_test_tables.csv")
